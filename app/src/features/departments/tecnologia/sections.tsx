...
export function TechResumen() {
  ...
  return (
    <>
      ...
      <Panel title="Uptime · últimos 30 días" action={<FilterTabs options={TIME_FILTERS} value={timeWindow} onChange={setTimeWindow} variant="time" />}>
        <Heatmap cells={HEAT} cols={15} legendLeft="22 abr — 21 may · 1 incidente menor (18 min)" />
      </Panel>

      <p className="mt-8 text-[13px] text-n-700 max-w-prose">
        Subsecciones: <b>Roadmap</b> (kanban), <b>Integraciones</b> (health + latencia con auto-refresh),
        <b> Audit log</b> (explorer).
      </p>
    </>
  )
}
