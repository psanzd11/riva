...
export function ScResumen() {
  ...
  return (
    <>
      <ScHeader actions={<><Button variant="outline">Inventario</Button><Button>+ OC</Button></>} />
      <KpiGrid>
      ...
      <Panel title="Stock por almacén" className="mb-8">
        <Donut
          slices={[
      ...
      <p className="text-[13px] text-n-700 max-w-prose">
        Subsecciones: <b>Inventario</b> (tabla SKUs + +OC sugerida), <b>Pipeline OC</b> (kanban drag&drop),
        <b> Proveedores</b> (lead times + tabla).
      </p>
    </>
  )
}
...
export function ScInventario() {
  ...
  return (
    <>
      <ScHeader actions={<><Button variant="outline">Exportar</Button><Button onClick={createPoFromCritical}>+ OC sugerida</Button></>} />
      ...
      <table className="data-table">
        <thead>
          <tr><th>SKU</th><th>Almacén</th><th>Stock</th><th>Umbral</th><th>Cobertura</th><th>OC sugerida</th><th>Estado</th></tr>
        </thead>
        <tbody>
          {skusByFilter.map((s) => {
...
export function ScOc() {
  return (
    <>
      <ScHeader actions={<><Button variant="outline">Exportar</Button><Button>+ OC</Button></>} />
      <div className="mb-4 flex items-baseline justify-between">
