...
const around = (base, spread) => Math.round(base + (rand() - 0.5) * 2 * spread)
...
// ============ USERS ============
const users = [
  { id: 'usr_ps', name: 'Pablo Sanz', email: 'ps@riva.com', role: 'ceo', sede: 'global', avatarColor: 'oak-mid', active: true },
  { id: 'usr_cm', name: 'Carla Méndez', email: 'cm@riva.com', role: 'comercial', sede: 'es', avatarColor: 'cove', active: true },
  { id: 'usr_jr', name: 'Jake Roberts', email: 'jr@riva.com', role: 'comercial', sede: 'us', avatarColor: 'oak-mid', active: true },
  { id: 'usr_da', name: 'Diego Arrieta', email: 'da@riva.com', role: 'comercial', sede: 'us', avatarColor: 'sage', active: true },
  { id: 'usr_at', name: 'Ashley Tan', email: 'at@riva.com', role: 'comercial', sede: 'us', avatarColor: 'cove-mid', active: true },
  { id: 'usr_lp', name: 'Laura Pinto', email: 'lp@riva.com', role: 'comercial', sede: 'es', avatarColor: 'cove-dark', active: true },
  { id: 'usr_mo', name: 'Marc Olivier', email: 'mo@riva.com', role: 'comercial', sede: 'us', avatarColor: 'cove-deepest', active: true },
  { id: 'usr_dc', name: 'Inés Vergara', email: 'iv@riva.com', role: 'director_comercial', sede: 'global', avatarColor: 'cove', active: true },
  { id: 'usr_dacc', name: 'Tomás Riera', email: 'tr@riva.com', role: 'director_accounting', sede: 'global', avatarColor: 'oak-mid', active: true },
  { id: 'usr_ops', name: 'Mateo Lara', email: 'ml@riva.com', role: 'operations_manager', sede: 'global', avatarColor: 'sage', active: true },
  { id: 'usr_mkt', name: 'Helena Costa', email: 'hc@riva.com', role: 'marketing_lead', sede: 'global', avatarColor: 'cove-mid', active: true },
  { id: 'usr_cs', name: 'Sara Vidal', email: 'sv@riva.com', role: 'customer_success', sede: 'global', avatarColor: 'oak-mid', active: true },
  { id: 'usr_tech', name: 'Andrés Bravo', email: 'ab@riva.com', role: 'tech_lead', sede: 'global', avatarColor: 'cove-dark', active: true },
]
...
let cmDealCount = PARTNER_DATA.length
...
partners = partners.map((p) => ({
  ...p,
  lastActivity: isoDaysAgo(p.lastActivityDays),
}))
// Remove helper
partners.forEach((p) => delete p.lastActivityDays)

// ============ DEALS ============
// Pipeline distribution: 42 cualif, 34 prop, 22 neg, 20 cierre/won, 8 lost (PLAN.md §4.2)
// Plus historic won: ~142 won YTD - 20 open won = ~122 closed-won prior
// Total: 42+34+22+20+8 = 126, but plan says ~118 open. We do 118 + historic
...
// Historic won: 142 - 20 = 122 extra won
for (let i = 0; i < 122; i++) deals.push(mkDeal('won'))

// Specific named deals from demo (override first ones for kanban realism)
const KAN_CARDS = [
...
// Inject named deals replacing first deals of each stage
...
const invoices = []
let invIdx = 0
const invNum = () => `2026-${String(400 + ++invIdx).padStart(4, '0')}`
// Most won deals get a paid invoice
const distribution = [
  { status: 'paid', bucket: 'current', count: 156 },
  { status: 'sent', bucket: '0_30', count: 14 },
  { status: 'overdue', bucket: '31_60', count: 8 },
  { status: 'overdue', bucket: '61_90', count: 4 },
  { status: 'overdue', bucket: '90_plus', count: 2 },
]
let dealCursor = 0
distribution.forEach(({ status, bucket, count }) => {
  for (let i = 0; i < count; i++) {
    const d = wonDeals[dealCursor % wonDeals.length]
    dealCursor++
    const ageDays = bucket === 'current' ? int(0, 14) : bucket === '0_30' ? int(0, 30) : bucket === '31_60' ? int(31, 60) : bucket === '61_90' ? int(61, 90) : int(91, 140)
    const issued = isoDaysAgo(ageDays + 30)
    const due = isoDaysAgo(ageDays)
    invoices.push({
      id: `inv_${String(invIdx).padStart(4, '0')}`,
      number: invNum(),
...
// Specific demo invoices
const DEMO_INV = [
  { number: '2026-0417', partnerName: 'Studio Rota', amount: 14200, currency: 'EUR', status: 'overdue', bucket: '0_30', ageDays: 7 },
  { number: '2026-0411', partnerName: 'UpWall Aspen', amount: 18400, currency: 'USD', status: 'overdue', bucket: '0_30', ageDays: 9 },
  { number: '2026-0402', partnerName: 'Mirage Boston', amount: 8400, currency: 'USD', status: 'overdue', bucket: '0_30', ageDays: 14 },
  { number: '2026-0398', partnerName: 'DuChâteau Bilbao', amount: 9600, currency: 'EUR', status: 'overdue', bucket: '0_30', ageDays: 18 },
  { number: '2026-0388', partnerName: 'Hakwood Sevilla', amount: 6840, currency: 'EUR', status: 'overdue', bucket: '0_30', ageDays: 22 },
  { number: '2026-0376', partnerName: 'FloorHaus Miami', amount: 4200, currency: 'USD', status: 'overdue', bucket: '31_60', ageDays: 34 },
]
DEMO_INV.forEach((di) => {
  const partner = partners.find((p) => p.name === di.partnerName)
  if (!partner) return
  invoices.push({
    id: `inv_demo_${di.number}`,
    number: di.number,
    partnerId: partner.id,
    amount: di.amount,
    currency: di.currency,
    issuedAt: isoDaysAgo(di.ageDays + 30),
    dueAt: isoDaysAgo(di.ageDays),
    status: di.status,
    agingBucket: di.bucket,
  })
})
...
// ============ SKUS ============
const SKU_NAMES = [
  { name: 'Cove', collection: 'TIERRA', finish: 'oak dark', warehouse: 'es', thresholdM2: 180, stockM2: 42 },
  { name: 'Laguna', collection: 'TIERRA', finish: 'oak light', warehouse: 'es', thresholdM2: 160, stockM2: 68 },
  { name: 'Velino', collection: 'TIERRA', finish: 'ash', warehouse: 'us', thresholdM2: 140, stockM2: 54 },
  { name: 'Glaze', collection: 'RIVA Spain', finish: 'oak light', warehouse: 'es', thresholdM2: 180, stockM2: 248 },
  { name: 'Vedetta', collection: 'RIVA Spain', finish: 'oak smoke', warehouse: 'us', thresholdM2: 140, stockM2: 184 },
  { name: 'Cotton', collection: 'TIERRA', finish: 'white oak', warehouse: 'es', thresholdM2: 120, stockM2: 320 },
  { name: 'Sand', collection: 'TIERRA', finish: 'natural', warehouse: 'us', thresholdM2: 100, stockM2: 280 },
  { name: 'Mercury', collection: 'RIVA Spain', finish: 'grey oak', warehouse: 'us', thresholdM2: 120, stockM2: 92 },
  { name: 'Earth', collection: 'TIERRA', finish: 'walnut', warehouse: 'es', thresholdM2: 110, stockM2: 152 },
  { name: 'Storm', collection: 'TIERRA', finish: 'charcoal', warehouse: 'es', thresholdM2: 100, stockM2: 142 },
]
const fillerSkus = []
for (let i = 0; i < 22; i++) {
  fillerSkus.push({
    name: `SKU-${i + 11}`,
    collection: pick(['TIERRA', 'RIVA Spain']),
    finish: pick(['oak light', 'oak dark', 'walnut', 'ash', 'grey oak']),
    warehouse: pick(['es', 'us']),
    thresholdM2: int(80, 200),
    stockM2: int(60, 400),
  })
}
const skus = [...SKU_NAMES, ...fillerSkus].map((sk, i) => ({
...
// ============ TICKETS ============
const tickets = [
  { num: '882', type: 'Defecto acabado', clientName: 'Casa Velázquez', partnerId: 'prt_flagship_mia', assigneeId: 'usr_cs', priority: 'high', status: 'open', slaHours: 24, openHoursAgo: 3 },
  { num: '881', type: 'Pedido recibido incompleto', clientName: 'Studio Rota', partnerId: 'prt_studio_rota', assigneeId: 'usr_da', priority: 'med', status: 'open', slaHours: 48, openHoursAgo: 6 },
  { num: '880', type: 'Consulta mantenimiento', clientName: 'UpWall Aspen', partnerId: 'prt_upwall_aspen', assigneeId: 'usr_at', priority: 'low', status: 'open', slaHours: 72, openHoursAgo: 12 },
  { num: '879', type: 'Reposición garantía', clientName: 'FloorHaus Miami', partnerId: 'prt_floorhaus_mia', assigneeId: 'usr_jr', priority: 'med', status: 'open', slaHours: 48, openHoursAgo: 24 },
  ...
  { num: '876', type: 'Consulta limpieza', clientName: 'Casa Mendel', partnerId: 'prt_casa_mendel', assigneeId: 'usr_lp', priority: 'low', status: 'open', slaHours: 96, openHoursAgo: 48 },
].map((t) => ({
  id: `tkt_${t.num}`,
  type: t.type,
...
const automationRules = [
  ...
  { id: 'ar_lead_nurture', name: 'Lead → secuencia email (sustituye HubSpot)', icon: 'M', description: 'Nuevo lead web · 4 emails en 21 días · score actualizado', trigger: { event: 'lead.created' }, conditions: [{ field: 'channel', op: 'eq', value: 'web' }], actions: [{ type: 'send_email_mock', params: { sequence: 'welcome' } }], active: true, runs: 142 },
  ...
  { id: 'ar_install_sched', name: 'Pago recibido → instalación propuesta', icon: 'O', description: 'Pago recibido · sugerir fecha de instalación según capacity', trigger: { event: 'payment.created' }, conditions: [], actions: [{ type: 'log', params: { suggest: 'installation' } }], active: true, runs: 64 },
...
const integrations = [
  ...
  { id: 'itg_hubspot', name: 'HubSpot', type: 'marketing', status: 'warn', statusLabel: '● Solo email', latencyMs: 220, lastSync: isoDaysAgo(0), config: {} },
