...
const HEAT_CELLS = [1, 2, 1, 3, 4, 2, 0, 1, 3, 2, 3, 3, 4, 0, 2, 2, 3, 4, 3, 2, 1, 1, 2, 4, 3, 2, 1, 0, 0, 1, 2, 2, -1, 1, 0] as const
...
export function OperationsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return installations.filter((i) => new Date(i.startAt) <= max).slice(0, 8)
  ...
  const crewLeaderboard = useMemo(() => {
    return crews.map((c) => {
      const projects = installations.filter((i) => i.crewId === c.id).length
      return { crew: c, projects }
  ...
  const timelineRows = filtered.map((ins) => {
    ...
    return {
      name: ins.project,
      sub: `${ins.sede.toUpperCase()} · ${ins.m2} m²`,
      startPct,
      widthPct,
      variant,
      date: dateShort(ins.startAt),
    }
  })
  ...
  return (
    <>
      ...
      <div className="mb-12 grid gap-8" style={{ gridTemplateColumns: '1fr 1fr 1fr' }}>
        ...
        <Panel title="Heatmap instalaciones · 5 semanas" action={<a className="link text-[11px] cursor-pointer">+ ver mes</a>}>
          <Heatmap
            cells={[...HEAT_CELLS] as (0 | 1 | 2 | 3 | 4 | -1)[]}
            cols={7}
            legendLeft="L · M · X · J · V · S · D"
          />
        </Panel>
      ...
      <div className="mb-4 flex items-baseline justify-between">
        <h2 className="font-display text-[26px] font-light tracking-[0.04em]">Equipo de instalación</h2>
        <FilterTabs options={[{ value: 'week', label: 'Sem.' }, { value: 'month', label: 'Mes' }, { value: 'ytd', label: 'YTD' }] as const} value="month" onChange={() => {}} variant="time" />
      </div>
      <table className="data-table">
        ...
        <tbody>
          ...
          {crewLeaderboard.length === 0 && <tr><td colSpan={7} className="text-center text-n-500">Sin equipos.</td></tr>}
        </tbody>
      </table>
        ...
        <Panel headless>
          <div className="p-6">
            <p className="mb-4 text-[13px] text-n-700">
              Cambiar crew de una instalación cambia su asignación en tiempo real. (Drag & drop completo entra en una iteración futura.)
            </p>
            <table className="data-table">
              <thead>
              ...
              <tbody>
                {filtered.map((ins) => (
                  <tr key={ins.id}>
                    ...
                    <td>
                      <Pill variant={ins.status === 'incident' ? 'err' : ins.status === 'done' ? 'ok' : 'warn'}>
                        {ins.status}
                      </Pill>
                    </td>
                  </tr>
